          ...
          <tbody>
            <tr v-for="(item, index) in filteredItems" :key="item.id"
              ...
              <td class="px-3 py-3 font-light" :title="item.sub_name">
                ...
                <!-- {{ checkImageFormCategory(item.sub_id) }}
                <img
                  :src="'/images/icons/101.png'"
                  alt="icon"
...
const { deleteUrbanIssue } = useUrbanIssues();
...
async function deleteItem(id) {
  try {
    await deleteUrbanIssue(id, "admin@gmail.com"); // Pass ID and email
...
async function fetchData(currentPage = 1) {
  try {
    const params = {
      page: currentPage,
      ...(showMine.value && { email: "admin@gmail.com" }),
    };
...
function fitBoundingBoxOnMap(bbox) {
  console.log("Trying to send bbox to MapLibre:", bbox);
  nextTick(() => {
    if (mapRef.value) {
      console.log("mapRef.value is ready");
...
function checkImageFormCategory(id) {
  for (const category of categoryList) {
    const subcategory = category.subcategories.find((sub) => sub.id === id);
    if (subcategory) {
      return subcategory.image || "not-found.png";
    }
  }
  return "not-found.png"; // Default image if not found
}

// Lifecycle Hook
onMounted(() => {
  console.log("mapRef onMounted:", mapRef.value); // Debug
  fetchData(1);
});
</script>
