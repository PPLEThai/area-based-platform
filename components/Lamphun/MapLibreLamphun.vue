  ...
  <div class="relative w-full h-full">
    ...
    <div class="absolute top-4 left-4 z-10 w-64 sm:w-96">
      <input type="text" v-model="searchQuery" @keydown.enter="performSearch"
        placeholder="ค้นหาสถานที่ เช่น ทางหลวงแผ่นดินหมายเลข 106" class="w-full p-2 border rounded-lg shadow" />
...
import { useLamphunBoundary } from "@/composables/useBoundary";
const { getLamphunBoundaryData } = useLamphunBoundary(); // Extract the function
const lamphunBoundaryData = getLamphunBoundaryData(); // Load the boundary data
